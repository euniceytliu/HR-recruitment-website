// Vercel Serverless Function - 智谱AI简历分析后端
// 部署到 Vercel 后，API 地址为：https://你的域名.vercel.app/api/analyze-resume
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (!ZHIPU_API_KEY) {
            console.error('❌ 未配置 ZHIPU_API_KEY 环境变量');
            return res.status(500).json({ 
                error: 'API Key 未配置，请联系管理员' 
            });
        }

        console.log('📝 开始分析简历:', fileName || '未知文件');
        console.log('📊 简历长度:', resumeText.length, '字符');

        // 构建完整的 Prompt（与前端保持一致）
        ...
        if (!response.ok) {
            ...
            throw new Error(`智谱AI API 调用失败: ${response.status}`);
        }

        const data = await response.json();
        console.log('✅ 智谱AI 返回:', JSON.stringify(data, null, 2));

        if (!data.choices || !data.choices[0] || !data.choices[0].message) {
            throw new Error('智谱AI 返回格式错误');
        }

        const aiResponse = data.choices[0].message.content;
        console.log('🤖 AI 分析结果:', aiResponse.substring(0, 200) + '...');
        ...
        try {
            ...
            console.log('原始内容:', aiResponse);
            throw new Error('AI 返回的不是有效的 JSON 格式');
        }

        // 验证结果格式
        if (!result.score || !result.type || !result.strengths || !result.improvements || !result.suggestions) {
            console.error('❌ 结果格式不完整:', result);
            throw new Error('AI 返回的结果格式不完整');
        }

        console.log('🎉 分析成功！分数:', result.score, '类型:', result.type);
        ...
        return res.status(500).json({
            success: false,
            error: error.message || '分析过程出现错误，请重试'
        });
    }
}

// 构建完整的 Prompt（与前端保持一致）
function buildPrompt(resumeText, fileName) {
    return `你是腾讯HR的资深招聘专家，拥有10年校招经验，深度理解腾讯文化和用人标准。
